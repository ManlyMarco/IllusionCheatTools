...
internal static class GameUtilities
{
    ...
    /// </summary>
    public static IEnumerable<Actor> GetCurrentActors(bool mainInstances)
    {
        if (InsideMaker)
        {
            if (mainInstances)
        ...
        if (HScene.IsActive())
        {
                return H.HScene._instance._hActorAll.AsEnumerable().Select(x=>x);
        }

        var talkManager = Manager.TalkManager._instance;
        if (talkManager != null && HResult.ADV.ADVManager._instance?.IsADV == true)
        {
